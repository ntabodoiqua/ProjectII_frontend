...
const LessonManagement = () => {
  ...
  const [selectedCourseIdForStats, setSelectedCourseIdForStats] =
    useState(null);
  ...
  const handleFilterSubmit = (values) => {
    ...
    if (newFilters.createdDate) {
      newFilters.createdFrom = newFilters.createdDate[0]
        .startOf("day")
        .toISOString();
      newFilters.createdTo = newFilters.createdDate[1]
        .endOf("day")
        .toISOString();
    }
    ...
    if (newFilters.updatedDate) {
      newFilters.updatedFrom = newFilters.updatedDate[0]
        .startOf("day")
        .toISOString();
      newFilters.updatedTo = newFilters.updatedDate[1]
        .endOf("day")
        .toISOString();
    }
        ...
        const moreMenuItems = [
          {
            ...
            onClick: () =>
              navigate(`/instructor/student-lesson-view/${record.id}`),
          },
          {
            key: "docs",
    ...
    try {
      ...
      if (apiResponse && typeof apiResponse.code !== "undefined") {
        if (apiResponse.code === 1000 && apiResponse.result) {
          ...
          message.error(
            apiResponse.message || "Không thể tải danh sách bài học từ API."
          );
        }
      } else {
        message.error(
          "Phản hồi không hợp lệ từ API khi tải danh sách bài học."
        );
      }
    } catch (error) {
  ...
  const handleViewDetails = async (lessonSummary) => {
    ...
    try {
      ...
      if (apiResponse && apiResponse.code === 1000 && apiResponse.result) {
        ...
        message.error(
          apiResponse?.message || "Không thể tải chi tiết bài học."
        );
  ...
  const handleEdit = async (lesson) => {
    ...
    try {
      ...
      if (apiResponse && apiResponse.code === 1000 && apiResponse.result) {
        ...
        message.error(
          apiResponse?.message || "Không thể tải chi tiết bài học để chỉnh sửa."
        );
  ...
  const handleDownloadLessonDocument = async (documentId, fileName) => {
    try {
      const response = await downloadLessonDocumentApi(
        selectedLessonForDocs.id,
        documentId
      );
  ...
  const handleOpenQuizStats = async (lesson) => {
    ...
    try {
      ...
      if (response.code === 1000 && response.result) {
        ...
        message.error(
          response.message ||
            "Không thể tải danh sách khóa học cho bài học này."
        );
        setCoursesForLesson([]);
      }
    } catch (error) {
  ...
  const handleCourseSelectForStats = async (courseId) => {
    ...
    if (courseId) {
      ...
      try {
        const response = await getLessonQuizStatsInCourseApi(
          selectedLessonForStats.id,
          courseId
        );
  ...
  return (
    <div>
          ...
          {loadingViewDetails ? (
            ...
            <Descriptions bordered column={1} size="small">
              <Descriptions.Item label="ID">
                {selectedLessonDetails.id}
              </Descriptions.Item>
              <Descriptions.Item label="Tên bài học">
                {selectedLessonDetails.title}
              </Descriptions.Item>
              <Descriptions.Item label="Số khóa học chứa bài học">
                {selectedLessonDetails.courseCount}
              </Descriptions.Item>
              <Descriptions.Item label="Mô tả ngắn">
                {selectedLessonDetails.description || "Chưa có mô tả"}
              </Descriptions.Item>
              <Descriptions.Item label="Nội dung chi tiết">
                {selectedLessonDetails.content || "N/A"}
              </Descriptions.Item>
              <Descriptions.Item label="Ngày tạo">
                {selectedLessonDetails.createdAt
                  ? new Date(selectedLessonDetails.createdAt).toLocaleString()
                  : "N/A"}
              </Descriptions.Item>
              <Descriptions.Item label="Cập nhật lần cuối">
                {selectedLessonDetails.updatedAt
                  ? new Date(selectedLessonDetails.updatedAt).toLocaleString()
                  : "N/A"}
              </Descriptions.Item>
              {selectedLessonDetails.createdBy && (
                <>
                  <Descriptions.Item label="Người tạo (Username)">
                    {selectedLessonDetails.createdBy.username}
                  </Descriptions.Item>
                  <Descriptions.Item label="Người tạo (Tên)">
                    {`${selectedLessonDetails.createdBy.lastName || ""} ${
                      selectedLessonDetails.createdBy.firstName || ""
                    }`.trim() || "N/A"}
                  </Descriptions.Item>
                  <Descriptions.Item label="Người tạo (Email)">
                    {selectedLessonDetails.createdBy.email || "N/A"}
                  </Descriptions.Item>
                </>
              )}
            </Descriptions>
      ...
      <Modal
        ...
        footer={[
          <Button
            key="upload"
            type="primary"
            onClick={handleUploadLessonDocument}
          >
          ...
          <Row gutter={16}>
            ...
            <Col span={8}>
              <Form.Item
                ...
                rules={[
                  { required: true, message: "Vui lòng nhập số lần thử!" },
                ]}
              >
                <InputNumber
                  min={1}
      ...
      <Drawer
        title={`Thống kê Quiz cho bài học: ${
          selectedLessonForStats?.title || ""
        }`}
        width={1000}
                ...
                <Table
                  ...
                  columns={[
                    ...
                    { title: "Quiz", dataIndex: "quizTitle", key: "quizTitle" },
                    {
                      ...
                      key: "score",
                      render: (score, record) =>
                        `${score.toFixed(1)}/${record.maxScore.toFixed(1)}`,
                    },
                    {
                      title: "Phần trăm",
