...
const LargeFileUpload = ({
  ...
  const validateFile = (file) => {
    ...
    if (file.size > maxSizeBytes) {
      return {
        isValid: false,
        error: `File quá lớn! Kích thước tối đa cho phép: ${maxSizeMB}MB. File hiện tại: ${formatFileSize(
          file.size
        )}`,
      };
    }
    ...
    if (!fileExtension || !allowedExtensions.includes(fileExtension)) {
      return {
        isValid: false,
        error: `Loại file không được hỗ trợ! Chỉ cho phép: PDF, DOC/DOCX, ảnh (JPG, PNG, GIF...) và video (MP4, AVI, MOV...). File hiện tại: .${
          fileExtension || "không rõ"
        }`,
      };
    }
    ...
    <div style={{ width: "100%" }}>
      ...
      <Dragger {...uploadProps} disabled={uploading}>
        <p className="ant-upload-drag-icon">
          <UploadOutlined
            style={{ fontSize: 48, color: uploading ? "#ccc" : "#1890ff" }}
          />
        </p>
        ...
        <p className="ant-upload-hint" style={{ color: "#666" }}>
          Hỗ trợ: PDF, DOC/DOCX, ảnh (JPG, PNG, GIF...), video (MP4, AVI,
          MOV...). Kích thước tối đa: {maxSizeMB}MB
        </p>
      </Dragger>
